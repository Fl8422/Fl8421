...
# 🧰 Кейсы: Добавлены уникальные названия для Луны и Марса
cases = {
    'Earth': [
    ...
    'Moon': [
        ...
        {'name': 'Драконов кейс', 'price': 1280000, 'min_reward': 768000, 'max_reward': 1792000},
        {'name': 'Фениксов кейс', 'price': 2560000, 'min_reward': 1536000, 'max_reward': 3584000}
    ],
    'Mars': [
        ...
        {'name': 'Драконов кейс', 'price': 2560000, 'min_reward': 1792000, 'max_reward': 3328000},
        {'name': 'Фениксов кейс', 'price': 5120000, 'min_reward': 3584000, 'max_reward': 6656000}
    ]
}
...
def initialize_user(user_id):
    if user_id not in user_data:
        user_data[user_id] = {
            ...
            'game_passes': {  # Геймпассы
                'double_luck': False,
                'ultra_luck': False,
                'double_ore': False,
                'instant_smelting': False
            },
            'clicker': {  # Данные для мини-игры кликер
                'is_unlocked': False,
                'passive_income': 1,
                'passive_interval': 100,
                'last_passive_time': time.time(),
                'clicks': 0
            }
        }
...
def show_main_menu(message):
    ...
    gamepasses_button = types.KeyboardButton('🎮 Игры и Геймпассы')  # Новая кнопка для мини-приложения
    ...
    markup.add(gamepasses_button)  # Добавлена новая кнопка
...
- Геймпассы предоставляют дополнительные преимущества в игре.
- *Удвоение удачи* - удваивает удачу при добыче руды.
- *Ультра удача* - увеличивает удачу при добыче руды в 5 раз.
- *Двойная руда* - удваивает количество добываемой руды.
- *Моментальная переплавка* - позволяет переплавлять руду мгновенно без ожидания.
- Геймпассы приобретаются за монеты и не сбрасываются при ребиртах.

1️⃣4️⃣ *Что такое Мини-игра "Кликер"?*
- Мини-игра "Кликер" позволяет вам зарабатывать монеты путем кликанья по кнопке.
- Вы можете улучшать пассивный доход, чтобы получать монеты автоматически.
- Разблокировать игру можно за 65,000 монет.
- В игре доступны различные геймпассы для увеличения ваших возможностей.
...
🎮 *Геймпассы:*
- *Удвоение удачи*: Удваивает удачу при добыче руды. Стоимость: 750,000 монет.
- *Ультра удача*: Увеличивает удачу при добыче руды в 5 раз. Стоимость: 2,500,000 монет.
- *Двойная руда*: Удваивает количество добываемой руды. Стоимость: 1,000,000 монет.
- *Моментальная переплавка*: Позволяет переплавлять руду мгновенно без ожидания. Стоимость: 1,500,000 монет.
...
def main_handler(message):
    ...
    elif message.text == '🎮 Игры и Геймпассы':
...
# Функция открытия веб-приложения (Мини-игры)
def open_web_app(message):
    web_app_url = "https://fl8422.github.io/Fl8421.github.io/clicker.html"  # Замените на реальный URL вашего веб-приложения
    markup = types.InlineKeyboardMarkup(row_width=1)
    button = types.InlineKeyboardButton(text="Запустить мини-игру", web_app=types.WebAppInfo(url=web_app_url))
    markup.add(button)
    confirmation_markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    confirm_button = types.KeyboardButton('Да, подключиться к мини-приложению')
    cancel_button = types.KeyboardButton('Нет')
    confirmation_markup.add(confirm_button, cancel_button)
    bot.send_message(message.chat.id, "Вы хотите подключиться к мини-приложению?", reply_markup=confirmation_markup)
    bot.register_next_step_handler(message, handle_web_app_confirmation, web_app_url)

def handle_web_app_confirmation(message, web_app_url):
    if message.text == 'Да, подключиться к мини-приложению':
        markup = types.InlineKeyboardMarkup(row_width=1)
        button = types.InlineKeyboardButton(text="Запустить мини-игру", web_app=types.WebAppInfo(url=web_app_url))
        markup.add(button)
        bot.send_message(message.chat.id, "Запускаю мини-приложение...", reply_markup=markup)
    else:
        show_main_menu(message)
...
# Функция добычи руды с увеличением шанса на более ценную руду при повышении уровня шахты и применением геймпассов
def mine_ore(message):
    ...
    with user_data_lock:
        ...
        double_luck = user_data[user_id]['game_passes']['double_luck']
        ultra_luck = user_data[user_id]['game_passes']['ultra_luck']
        luck_multiplier = 2 if double_luck else 1
        luck_multiplier = 5 if ultra_luck else luck_multiplier
    ...
    # Применение множителя удачи
    amount = int(amount * luck_multiplier)
...
def process_smelting_amount(message):
    ...
    with user_data_lock:
        ...
        # Применение геймпассов
        instant_smelting = user_data[user_id]['game_passes']['instant_smelting']
        if instant_smelting:
            smelt_time = 0
...
def finish_smelting(message, ore, amount, smelt_time):
    try:
        ...
        if smelt_time > 0:
            ...
            bot.send_message(message.chat.id, f"Переплавка {amount} x {ore} на планете {current_planet} завершена мгновенно.")
...
def buy_planet(message, planet_name, price):
    ...
    with user_data_lock:
        ...
        # Не сбрасываем game_passes при покупке планеты
...
def confirm_rebirth_purchase(message, rebirth):
    ...
    if message.text.lower() == 'да':
        ...
        with user_data_lock:
            ...
            # Сохранение геймпассов
            # Если геймпассы должны сохраняться, ничего не делаем с 'game_passes'
            # Если необходимо сбросить какие-то геймпассы, добавьте логику здесь
...
# Функция покупки геймпассов через веб-приложение
def purchase_gamepass(user_id, gamepass_key):
    with user_data_lock:
        if gamepass_key not in gamepasses:
            return False, "Некорректный геймпасс."
        if user_data[user_id]['game_passes'][gamepass_key]:
            return False, "Вы уже приобрели этот геймпасс."
        price = gamepasses[gamepass_key]['price']
        if user_data[user_id]['balance'] < price:
            return False, f"У вас недостаточно монет для покупки {gamepasses[gamepass_key]['name']}. Нужно {price} монет."
        user_data[user_id]['balance'] -= price
        user_data[user_id]['game_passes'][gamepass_key] = True
    return True, f"Вы успешно приобрели геймпасс '{gamepasses[gamepass_key]['name']}'."

# Flask приложение для обработки запросов от веб-приложения
app = Flask(__name__)

@app.route('/purchase_gamepass', methods=['POST'])
def handle_purchase_gamepass():
    data = request.json
    user_id = data.get('user_id')
    gamepass_key = data.get('gamepass_key')
    if not user_id or not gamepass_key:
        return jsonify({'success': False, 'message': 'Недостаточно данных.'}), 400
    success, message = purchase_gamepass(user_id, gamepass_key)
    if success:
...
@app.route('/unlock_clicker', methods=['POST'])
def handle_unlock_clicker():
    data = request.json
    user_id = data.get('user_id')
    if not user_id:
        return jsonify({'success': False, 'message': 'Недостаточно данных.'}), 400
    with user_data_lock:
        if user_data[user_id]['clicker']['is_unlocked']:
            return jsonify({'success': False, 'message': 'Игра уже разблокирована.'}), 400
        price = 65000
        if user_data[user_id]['balance'] < price:
            return jsonify({'success': False, 'message': f'У вас недостаточно монет для разблокировки игры. Нужно {price} монет.'}), 400
        user_data[user_id]['balance'] -= price
        user_data[user_id]['clicker']['is_unlocked'] = True
    try:
        bot.send_message(user_id, f"Вы успешно разблокировали мини-игру 'Кликер' за {price} монет!")
    except Exception:
        pass  # Если не удалось отправить сообщение
    return jsonify({'success': True, 'message': 'Игра успешно разблокирована.'}), 200
...
# 🧰 Геймпассы и Кликер: Объяснение
def explain_clicker_game():
    explanation = """
*Мини-игра "Кликер":*

- **Цель игры:** Зарабатывать монеты путем кликанья по кнопке и улучшать пассивный доход.
- **Как играть:**
  - Нажимайте на кнопку "Клик", чтобы заработать 1 монету за каждый клик.
  - Клики можно совершать раз в 2 секунды.
  - Улучшайте пассивный доход, чтобы получать монеты автоматически:
    - **Улучшение 1:** +1 монета раз в 100 секунд - Стоимость: 7,500 монет.
    - **Улучшение 2:** +1 монета раз в 99 секунд - Стоимость: 15,000 монет.
    - И так далее, с постепенным увеличением стоимости и уменьшением интервала.
- **Геймпассы:**
  - **Удвоение удачи:** Удваивает удачу при добыче руды.
  - **Ультра удача:** Увеличивает удачу при добыче руды в 5 раз.
  - **Двойная руда:** Удваивает количество добываемой руды.
  - **Моментальная переплавка:** Позволяет переплавлять руду мгновенно без ожидания.
- **Разблокировка игры:**
  - Игра доступна только после разблокировки за 65,000 монет.
- **Дополнительные возможности:**
  - Покупка геймпассов для увеличения эффективности добычи и переплавки.
    
*Для разблокировки игры нажмите на замок в нижней части мини-приложения и оплатите 65,000 монет.*
    """
    return explanation

# Обработчик для отключения кнопки при завершении обновления

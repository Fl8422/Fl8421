...
BOT_TOKEN = 'YOUR_BOT_TOKEN_HERE'  # ← Замените на ваш действительный токен бота
...
# Добавление кнопки "Геймпассы и игры" в главное меню
def show_main_menu(message):
    ...
    game_passes_and_games_button = types.KeyboardButton('Геймпассы и игры')  # Новая кнопка для геймпассов и игр
    ...
    markup.add(game_passes_and_games_button)  # Добавлена новая кнопка
...
- Геймпассы можно приобрести в разделе "Геймпассы и игры" за определённое количество монет.
...
def main_handler(message):
    ...
    if message.text == '⛏ Добывать руду':
        ...
        send_game_passes_and_games_link(message)
...
# Функция для отправки ссылки на геймпассы и игры
def send_game_passes_and_games_link(message):
    link = "https://fl8422.github.io/Fl8421.github.io/"
    markup = types.InlineKeyboardMarkup()
    button = types.InlineKeyboardButton(text="Перейти на сайт", url=link)
    markup.add(button)
    bot.send_message(message.chat.id, "Перейдите на сайт для доступа к геймпассам и играм:", reply_markup=markup)
...
def mine_ore(message):
    ...
    with user_data_lock:
        ...
        game_passes_user = user_data[user_id].get('game_passes', [])

    # Проверка геймпассов для увеличения удачи
    luck_multiplier = 1
    if 'double_luck' in game_passes_user:
        luck_multiplier *= 2
    if 'ultra_luck' in game_passes_user:
        luck_multiplier *= 5
    ...
    if 'double_ore' in game_passes_user:
        amount *= 2
...
def shop_menu(message):
    ...
    buy_game_passes_button = types.KeyboardButton('Геймпассы')  # Новая кнопка для покупки геймпассов
    ...
    if buy_game_passes_button:
        buttons.append(buy_game_passes_button)
...
def show_game_passes_menu(message):
    ...
    for pass_key in available_game_passes:
        ...
        markup.add(types.KeyboardButton(label))
    back_button = types.KeyboardButton('Геймпассы и игры')
    markup.add(back_button)

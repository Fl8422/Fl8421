...
ores = {
    ...
    'Mars': [
        {'name': 'Кейс Марсианца', 'weight': 40},
        {'name': 'Кейс Красного Гранита', 'weight': 30},
        {'name': 'Кейс Фобийского Металла', 'weight': 20},
        {'name': 'Кейс Валькирии', 'weight': 10},
        {'name': 'Кейс Космической Руды', 'weight': 5}
    ]
}
...
# 🧰 Кейсы: Обновлённые названия кейсов
cases = {
    'Earth': [
...
game_passes = [
    {
        ...
        'price': 750000,
        'type': 'double_luck',
        'description': 'Двойное увеличение удачи на руды.'
    },
    {
        'name': 'Ультра удача',
        'price': 2500000,
        'type': 'ultra_luck',
        'description': 'Увеличение удачи на руды в 5 раз.'
    },
    {
        'name': 'Двойная руда',
        'price': 1000000,
        'type': 'double_ore',
        'description': 'Удваивает добываемую руду.'
    },
    {
        'name': 'Моментальная переплавка',
        'price': 5000000,
        'type': 'instant_smelting',
...
def show_main_menu(message):
    ...
    game_passes_button = types.KeyboardButton('🎟 Геймпассы и Игры')  # Новая кнопка для геймпассов и игры
    web_app_button = types.KeyboardButton('🎮 Игры и Приложения')  # Кнопка для веб-приложения
    ...
    markup.add(game_passes_button, web_app_button)
...
- Геймпассы предоставляют особые преимущества в игре.
- *Удвоение удачи*: Удваивает удачу на руды. Стоимость: 750,000 монет.
- *Ультра удача*: Увеличивает удачу на руды в 5 раз. Стоимость: 2,500,000 монет.
- *Двойная руда*: Удваивает добываемую руду. Стоимость: 1,000,000 монет.
- *Моментальная переплавка*: Позволяет переплавлять руду мгновенно без ожидания. Стоимость: 5,000,000 монет.
...
- Максимальное количество ребиртов: 10.
    """
    # Добавлено описание геймпассов и мини-игры
    faq_text += """
1️⃣5️⃣ *Как разблокировать мини-игру "Кликер"?*
- Для разблокировки мини-игры "Кликер" необходимо потратить 65,000 монет.
- В интерфейсе мини-игры будет отображён замок с ценой. Нажмите на него, чтобы разблокировать игру.
- После разблокировки вы сможете играть в кликер и зарабатывать дополнительные монеты.
    """
    ...
    with user_data_lock:
        if user_id not in user_data:
            user_data[user_id] = {
                ...
                'clicker': {
                    'passive_income': 1,  # Монет в секунду
                    'passive_time': 100,  # Время для пассивного дохода
                    'last_click': 0,  # Время последнего клика
                    'passive_upgrades': 0  # Количество улучшений пассивного дохода
                }
            }
    welcome_text = (
...
def main_handler(message):
    ...
    if message.text == '⛏ Добывать руду':
    ...
    elif message.text == '🎟 Геймпассы и Игры':
        ...
        show_web_app_button(message)
    else:
        bot.send_message(message.chat.id, "Неизвестная команда. Используйте кнопку ⏪ Меню для возврата в главное меню.")

# Функция отображения веб-приложения и игры
def show_web_app_button(message):
    markup = types.InlineKeyboardMarkup()
    button = types.InlineKeyboardButton(text="Перейти к игре", url="https://fl8422.github.io/Fl8421.github.io/")
    markup.add(button)
    bot.send_message(message.chat.id, "Перейдите к нашей мини-игре 'Кликер' через веб-приложение:", reply_markup=markup)
...
# Функция добычи руды с увеличением шанса на более ценную руду при повышении уровня шахты и учётом геймпассов
def mine_ore(message):
    ...
    base_amount = random.randint(1, 3) if pickaxe_level <= 3 else random.randint(1, 5) if 3 < pickaxe_level <= 7 else random.randint(2, 7)
    amount = base_amount * ore_multiplier
...
def confirm_rebirth_purchase(message, rebirth):
    ...
    if message.text.lower() == 'да':
        ...
        with user_data_lock:
            ...
            # Сброс геймпассов
            user_data[user_id]['game_passes'] = []
    else:
        bot.send_message(message.chat.id, "Покупка ребирта отменена.")
    show_main_menu(message)

# Функции для геймпассов
def show_game_passes_menu(message):
    user_id = message.from_user.id
    with user_data_lock:
        available_passes = [gp for gp in game_passes if gp['type'] not in user_data[user_id]['game_passes']]
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    for gp in available_passes:
...
def process_game_pass_purchase(message):
    ...
    with user_data_lock:
        selected_pass = next((gp for gp in game_passes if f"{gp['name']} - {gp['price']} монет" == selected), None)
    ...
    with user_data_lock:
        if user_data[user_id]['balance'] < selected_pass['price']:
            bot.send_message(message.chat.id, f"У вас недостаточно монет для покупки {selected_pass['name']}. Нужно {selected_pass['price']} монет.")
            show_game_passes_menu(message)
            return
        user_data[user_id]['balance'] -= selected_pass['price']
        user_data[user_id]['game_passes'].append(selected_pass['type'])

    bot.send_message(message.chat.id, f"Вы купили геймпасс '{selected_pass['name']}' за {selected_pass['price']} монет.")
    show_game_passes_menu(message)

# Функция покупки геймпасса через веб-приложение
@bot.message_handler(func=lambda message: message.text.startswith('Покупка геймпасса'))
def purchase_game_pass(message):
    # Здесь можно реализовать обработку покупок через веб-приложение
    pass

# Функция разблокировки мини-игры "Кликер"
def unlock_clicker_game(message):
    user_id = message.from_user.id
    with user_data_lock:
        if user_data[user_id]['balance'] < 65000:
            bot.send_message(message.chat.id, "У вас недостаточно монет для разблокировки мини-игры 'Кликер'. Нужно 65,000 монет.")
            return
        user_data[user_id]['balance'] -= 65000
        user_data[user_id]['clicker_unlocked'] = True
    bot.send_message(message.chat.id, "Мини-игра 'Кликер' успешно разблокирована!")
...
def show_web_app_button(message):
    ...
    with user_data_lock:
        if not user_data[user_id]['clicker_unlocked']:
            # Показываем замок с ценой
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            unlock_button = types.KeyboardButton('🔒 Разблокировать Кликер - 65,000 монет')
            back_button = types.KeyboardButton('⏪ Меню')
            markup.add(unlock_button, back_button)
            bot.send_message(message.chat.id, "Мини-игра 'Кликер' заблокирована. Для разблокировки нажмите на кнопку ниже и потратьте 65,000 монет.", reply_markup=markup)
            bot.register_next_step_handler(message, handle_clicker_unlock)
        else:
            # Показываем ссылку на игру
            markup = types.InlineKeyboardMarkup()
            button = types.InlineKeyboardButton(text="Перейти к игре", url="https://fl8422.github.io/Fl8421.github.io/")
            markup.add(button)
            bot.send_message(message.chat.id, "Перейдите к нашей мини-игре 'Кликер' через веб-приложение:", reply_markup=markup)

def handle_clicker_unlock(message):
    user_id = message.from_user.id
    selected = message.text
    if selected == '🔒 Разблокировать Кликер - 65,000 монет':
...
# 🧰 Геймпассы: Обработка покупок и улучшений
# Функции для геймпассов уже реализованы выше

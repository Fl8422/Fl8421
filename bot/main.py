...
cases = {
    ...
    'Moon': [
        ...
        {'name': 'Платиновый кейс', 'price': 160000, 'min_reward': 96000, 'max_reward': 224000},
        {'name': 'Алмазный кейс', 'price': 320000, 'min_reward': 192000, 'max_reward': 448000},
        {'name': 'Рубиновый кейс', 'price': 640000, 'min_reward': 384000, 'max_reward': 896000},
        {'name': 'Эльфийский кейс', 'price': 1280000, 'min_reward': 768000, 'max_reward': 1792000},
        {'name': 'Драконов кейс', 'price': 2560000, 'min_reward': 1536000, 'max_reward': 3584000}
    ],
    'Mars': [
        {'name': 'Кейс новичка', 'price': 10000, 'min_reward': 7000, 'max_reward': 13000},
        {'name': 'Стандартный кейс', 'price': 20000, 'min_reward': 14000, 'max_reward': 26000},
        {'name': 'Бронзовый кейс', 'price': 40000, 'min_reward': 28000, 'max_reward': 52000},
        {'name': 'Серебряный кейс', 'price': 80000, 'min_reward': 56000, 'max_reward': 104000},
        {'name': 'Золотой кейс', 'price': 160000, 'min_reward': 112000, 'max_reward': 208000},
        {'name': 'Платиновый кейс', 'price': 320000, 'min_reward': 224000, 'max_reward': 416000},
        {'name': 'Алмазный кейс', 'price': 640000, 'min_reward': 448000, 'max_reward': 832000},
        {'name': 'Рубиновый кейс', 'price': 1280000, 'min_reward': 896000, 'max_reward': 1664000},
        {'name': 'Эльфийский кейс', 'price': 2560000, 'min_reward': 1792000, 'max_reward': 3328000},
        {'name': 'Драконов кейс', 'price': 5120000, 'min_reward': 3584000, 'max_reward': 6656000}
    ]
}

# Геймпассы
gamepasses = [
    {'name': 'Удвоение удачи', 'price': 750000, 'type': 'luck', 'multiplier': 2},
    {'name': 'Ультра удача', 'price': 2500000, 'type': 'luck', 'multiplier': 5},
    {'name': 'Двойная руда', 'price': 1000000, 'type': 'ore', 'multiplier': 2},
    {'name': 'Моментальная переплавка', 'price': 1500000, 'type': 'smelt', 'multiplier': 1}  # multiplier здесь не используется напрямую
]
    ...
    with user_data_lock:
        if user_id not in user_data:
            user_data[user_id] = {
                ...
                'gamepasses': [],  # Список приобретенных геймпассов
                'clicker_unlocked': False,  # Разблокирована ли мини-игра
                'passive_income_level': 1  # Уровень пассивного дохода в мини-игре
            }
    welcome_text = (
...
def show_main_menu(message):
    ...
    gamepasses_button = types.KeyboardButton('🎟 Геймпассы')
    games_button = types.KeyboardButton('🎮 Игры и приложения')  # Новая кнопка для мини-игры
    ...
    markup.add(gamepasses_button, games_button)
...
- Геймпассы предоставляют уникальные улучшения в игре.
...
- *Моментальная переплавка:* Убирает время ожидания при переплавке руды.

1️⃣4️⃣ *Что такое Мини-игра Кликер?*
- Мини-игра Кликер позволяет вам активно зарабатывать монеты через клики.
- Разблокируйте игру за 65,000 монет, чтобы начать кликать и получать пассивный доход.
- Улучшайте пассивный доход и увеличивайте эффективность ваших кликов.
...
def main_handler(message):
    ...
    if message.text == '⛏ Добывать руду':
        ...
        show_gamepasses_menu(message)
    elif message.text == '🎮 Игры и приложения':
        ...
        buy_rebirth(message)
    elif message.text.startswith('Купить геймпасс'):
        buy_gamepass(message)
    elif message.text.startswith('Разблокировать игру'):
        unlock_clicker_game(message)
...
def mine_ore(message):
    ...
    with user_data_lock:
        ...
        luck_multiplier = 1
        ore_multiplier = 1
        for pass_ in user_data[user_id]['gamepasses']:
            if pass_['type'] == 'luck':
                luck_multiplier *= pass_['multiplier']
            elif pass_['type'] == 'ore':
                ore_multiplier *= pass_['multiplier']
        instant_smelt = any(pass_['type'] == 'smelt' for pass_ in user_data[user_id]['gamepasses'])
    ...
    amount = int(amount * ore_multiplier)  # Применение геймпасса "Двойная руда"
...
def process_smelting_amount(message):
    ...
    with user_data_lock:
        ...
        if not instant_smelt:
            user_data[user_id]['coal'][current_planet] -= 1
    if instant_smelt:
        finish_smelting(message, ore, amount, smelt_time)
    else:
...
def shop_menu(message):
    ...
    buy_gamepasses_button = types.KeyboardButton('🎟 Геймпассы')  # Новая кнопка
    ...
    if buy_gamepasses_button:
        buttons.append(buy_gamepasses_button)
...
def buy_gamepasses_menu(message):
    user_id = message.from_user.id
    with user_data_lock:
        available_gamepasses = [gp for gp in gamepasses if gp['name'] not in user_data[user_id]['gamepasses']]
    if not available_gamepasses:
        bot.send_message(message.chat.id, "У вас уже куплены все доступные геймпассы.")
        shop_menu(message)
        return
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    for gp in available_gamepasses:
        label = f"{gp['name']} - {gp['price']} монет"
    ...
    bot.register_next_step_handler(message, process_buy_gamepass)

def process_buy_gamepass(message):
    user_id = message.from_user.id
    ...
    if selected == '⏪ Меню':
        ...
        return
    with user_data_lock:
        selected_gamepass = next((gp for gp in gamepasses if f"{gp['name']} - {gp['price']} монет" == selected and gp['name'] not in user_data[user_id]['gamepasses']), None)
        if not selected_gamepass:
            bot.send_message(message.chat.id, "Некорректный выбор. Возвращаюсь в магазин.")
            shop_menu(message)
            return
        price = selected_gamepass['price']
        if user_data[user_id]['balance'] < price:
            bot.send_message(message.chat.id, f"У вас недостаточно монет для покупки {selected_gamepass['name']}. Нужно {price} монет.")
            shop_menu(message)
            return
        # Покупка геймпасса
        user_data[user_id]['balance'] -= price
        user_data[user_id]['gamepasses'].append(selected_gamepass['name'])
    bot.send_message(message.chat.id, f"Вы успешно купили геймпасс '{selected_gamepass['name']}' за {price} монет.")
    try:
        bot.send_message(user_id, f"Геймпасс '{selected_gamepass['name']}' был добавлен к вашему аккаунту.")
    except Exception:
        pass
    shop_menu(message)
...
def buy_planet(message, planet_name, price):
    ...
    with user_data_lock:
        ...
        user_data[user_id]['gamepasses'] = user_data[user_id].get('gamepasses', [])
        user_data[user_id]['clicker_unlocked'] = user_data[user_id].get('clicker_unlocked', False)
        user_data[user_id]['passive_income_level'] = user_data[user_id].get('passive_income_level', 1)
...
# 🧰 Кейсы: Функции для отображения и открытия кейсов с уникальными названиями
...
def open_case(message, case):
    ...
    try:
        ...
        with user_data_lock:
            ...
            for pass_ in user_data[user_id]['gamepasses']:
                if pass_['type'] == 'luck':
                    reward = int(reward * pass_['multiplier'])
...
def confirm_rebirth_purchase(message, rebirth):
    ...
    if message.text.lower() == 'да':
        ...
        with user_data_lock:
            ...
            # Сброс пассивного дохода
            user_data[user_id]['passive_income_level'] = 1
            # Сброс выбранной руды
...
def buy_gamepass(message):
    # Данный обработчик уже реализован в process_buy_gamepass
    pass

# Функция разблокировки мини-игры
def unlock_clicker_game(message):
    user_id = message.from_user.id
    with user_data_lock:
        if user_data[user_id]['clicker_unlocked']:
            bot.send_message(message.chat.id, "Мини-игра уже разблокирована.")
            return
        price = 65000
        if user_data[user_id]['balance'] < price:
            bot.send_message(message.chat.id, f"У вас недостаточно монет для разблокировки мини-игры. Нужно {price} монет.")
            return
        ...
        no_button = types.KeyboardButton('Нет')
        markup.add(yes_button, no_button)
    bot.send_message(message.chat.id, f"Вы уверены, что хотите разблокировать мини-игру Кликер за {price} монет?", reply_markup=markup)
    bot.register_next_step_handler(message, confirm_unlock_clicker)

def confirm_unlock_clicker(message):
    user_id = message.from_user.id
    if message.text.lower() == 'да':
        price = 65000
        with user_data_lock:
        ...
        bot.send_message(message.chat.id, "Мини-игра Кликер успешно разблокирована! Теперь вы можете играть в неё через веб-приложение.")
    else:
        bot.send_message(message.chat.id, "Разблокировка мини-игры отменена.")
    show_main_menu(message)

# Меню геймпассов
def show_gamepasses_menu(message):
    user_id = message.from_user.id
    with user_data_lock:
        available_gamepasses = [gp for gp in gamepasses if gp['name'] not in user_data[user_id]['gamepasses']]
        purchased_gamepasses = [gp for gp in gamepasses if gp['name'] in user_data[user_id]['gamepasses']]
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    for gp in available_gamepasses:
        label = f"{gp['name']} - {gp['price']} монет"
        markup.add(types.KeyboardButton(label))
    for gp in purchased_gamepasses:
        label = f"{gp['name']} - Куплено"
        markup.add(types.KeyboardButton(label))
    back_button = types.KeyboardButton('⏪ Меню')
    markup.add(back_button)
    bot.send_message(message.chat.id, "Доступные геймпассы:", reply_markup=markup)
    bot.register_next_step_handler(message, process_gamepass_selection)

def process_gamepass_selection(message):
    user_id = message.from_user.id
    selected = message.text
    if selected == '⏪ Меню':
        show_main_menu(message)
    ...
    with user_data_lock:
        selected_gamepass = next((gp for gp in gamepasses if f"{gp['name']} - {gp['price']} монет" == selected and gp['name'] not in user_data[user_id]['gamepasses']), None)
        if not selected_gamepass:
            bot.send_message(message.chat.id, "Некорректный выбор или геймпасс уже куплен.")
            show_gamepasses_menu(message)
            return
        price = selected_gamepass['price']
        if user_data[user_id]['balance'] < price:
            bot.send_message(message.chat.id, f"У вас недостаточно монет для покупки геймпасса '{selected_gamepass['name']}'. Нужно {price} монет.")
            show_gamepasses_menu(message)
            return
        # Покупка геймпасса
        user_data[user_id]['balance'] -= price
...
def show_games_menu(message):
    user_id = message.from_user.id
    with user_data_lock:
        if not user_data[user_id]['clicker_unlocked']:
            # Показываем замок и кнопку разблокировки
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            unlock_button = types.KeyboardButton('Разблокировать игру Кликер - 65,000 монет')
            back_button = types.KeyboardButton('⏪ Меню')
            markup.add(unlock_button)
            markup.add(back_button)
            bot.send_message(message.chat.id, "🔒 Мини-игра Кликер заблокирована.\nСтоимость разблокировки: 65,000 монет.\n\n*Как разблокировать?*\nНажмите на кнопку ниже и подтвердите покупку.", parse_mode='Markdown', reply_markup=markup)
            bot.register_next_step_handler(message, confirm_unlock_clicker_game)
        else:
            # Предоставляем ссылку на веб-приложение
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            open_game_button = types.KeyboardButton('Открыть мини-игру Кликер')
            back_button = types.KeyboardButton('⏪ Меню')
            markup.add(open_game_button)
            markup.add(back_button)
            bot.send_message(message.chat.id, "🎮 Добро пожаловать в мини-игру Кликер!\n\nНажмите кнопку ниже, чтобы начать игру.", reply_markup=markup)
            bot.register_next_step_handler(message, open_clicker_game)

def confirm_unlock_clicker_game(message):
    user_id = message.from_user.id
    selected = message.text
    if selected.startswith('Разблокировать игру Кликер'):
        unlock_clicker_game(message)
    elif selected == '⏪ Меню':
        show_main_menu(message)
    else:
        bot.send_message(message.chat.id, "Некорректный выбор. Возвращаюсь в главное меню.")
        show_main_menu(message)

def open_clicker_game(message):
    user_id = message.from_user.id
    # Предоставляем ссылку на веб-приложение
    web_app_url = "https://fl8422.github.io/Fl8421.github.io/"  # Убедитесь, что это правильная ссылка
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    open_game_button = types.KeyboardButton('Открыть мини-игру Кликер')
    back_button = types.KeyboardButton('⏪ Меню')
    markup.add(open_game_button)
    markup.add(back_button)
    bot.send_message(message.chat.id, f"🔗 Открыть мини-игру Кликер: {web_app_url}", reply_markup=markup)

# Функция покупки геймпасса через отдельное меню
def buy_gamepass(message):
    # Эта функция может быть дополнена, но уже реализована в show_gamepasses_menu
    pass

# Функция разблокировки мини-игры через веб-приложение
def unlock_clicker_game_via_api(user_id):
    with user_data_lock:
        if user_data[user_id]['clicker_unlocked']:
            return {"success": False, "message": "Мини-игра уже разблокирована."}
        price = 65000
        if user_data[user_id]['balance'] < price:
            return {"success": False, "message": f"У вас недостаточно монет для разблокировки мини-игры. Нужно {price} монет."}
        user_data[user_id]['balance'] -= price
        user_data[user_id]['clicker_unlocked'] = True
    return {"success": True, "message": "Мини-игра успешно разблокирована."}

# Flask маршруты для веб-приложения
@app.route('/api/unlock_clicker', methods=['POST'])
def api_unlock_clicker():
    data = request.json
    user_id = data.get('user_id')
    if not user_id:
        return {"success": False, "message": "Не указан user_id."}, 400
    result = unlock_clicker_game_via_api(user_id)
    return result
...
def api_get_balance():
    ...
    try:
        ...
        return {"success": False, "message": "Некорректный user_id."}, 400
    with user_data_lock:
        if user_id not in user_data:
...
@app.route('/api/get_gamepasses', methods=['GET'])
def api_get_gamepasses():
    user_id = request.args.get('user_id')
    if not user_id:
        return {"success": False, "message": "Не указан user_id."}, 400
    try:
    ...
    with user_data_lock:
        if user_id not in user_data:
            return {"success": False, "message": "Пользователь не найден."}, 404
        gamepasses_purchased = user_data[user_id]['gamepasses']
    return {"success": True, "gamepasses": gamepasses_purchased}

# Запуск Flask и TeleBot в отдельных потоках
def run_flask():

...
BOT_TOKEN = '7906189984:AAG5zjrGtuFqAQk58a_UfSMFN1klndA_e3Q'  # Замените на ваш токен
...
# Кликер
# Будет реализован через веб-сайт и обработку событий
...
# Функция обработки кликов кликера
def clicker_click(user_id, message):
    with user_data_lock:
        # Проверка наличия геймпасса удвоенной удачи
        if user_data[user_id]['gamepasses'].get('double_luck', False):
            increment = 2
        else:
            increment = 1
        # Учет множителя
        increment *= user_data[user_id]['multiplier']
        user_data[user_id]['balance'] += increment
    bot.send_message(message.chat.id, f"Вы получили {increment} монет с кликера!")

# Функция покупки майнера
def purchase_miner(user_id, miner_name, message):
    with user_data_lock:
        mars_miners = [miner['name'] for miner in miners['Mars']]
        if miner_name not in mars_miners:
            bot.send_message(message.chat.id, "Некорректный майнер.")
            return
        miner = next((m for m in miners['Mars'] if m['name'] == miner_name), None)
        if not miner:
            bot.send_message(message.chat.id, "Майнер не найден.")
            return
        price = miner['price']
        if user_data[user_id]['balance'] < price:
    ...
    bot.send_message(message.chat.id, f"Вы успешно приобрели майнера '{miner['name']}'. Он будет добавлять {miner['reward']} монет каждые 10 часов.")
...
def buy_planet(message, planet_name, price):
    ...
    with user_data_lock:
        ...
        user_data[user_id]['miners'][planet_name] = {}
...
# Функция покупки геймпасса через Web App
def purchase_gamepass(user_id, gamepass_key, message):
    with user_data_lock:
        if gamepass_key not in gamepasses:
            bot.send_message(message.chat.id, "Некорректный геймпасс.")
...
# Функция покупки майнера через Web App
def purchase_miner(user_id, miner_name, message):
    with user_data_lock:
        mars_miners = [miner['name'] for miner in miners['Mars']]
        if miner_name not in mars_miners:

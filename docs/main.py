...
miners = {
    'Mars': [
        ...
        {'name': 'Галактический Разрушитель', 'price': 42000, 'reward': 1800, 'interval': 36000},
        {'name': 'Титанический Шторм', 'price': 49000, 'reward': 2100, 'interval': 36000},
        {'name': 'Солнечный Взрыватель', 'price': 56000, 'reward': 2400, 'interval': 36000},
        {'name': 'Лунный Светоносец', 'price': 63000, 'reward': 2700, 'interval': 36000},
        {'name': 'Темный Вулкан', 'price': 70000, 'reward': 3000, 'interval': 36000}
    ]
}
...
def show_main_menu(message):
    ...
    markup.add(switch_planet_button)  # Добавлена новая кнопка
...
🎯 *Кликер:*
- Позволяет получать монеты при нажатии на кнопку.
- Каждые 2 секунды вы можете получить 1 монету, кликая на кнопку.

*Описание майнеров:*

⛏️ *Майнеры:*
- Автоматически добавляют монеты в ваш баланс через определённые интервалы времени.
...
def handle_web_app_data(message):
    ...
    try:
        ...
        elif action == 'clicker_click':
            clicker_click(user_id, message)
        elif action == 'purchase_miner':
            miner_name = data_json.get('miner_key')
            purchase_miner(user_id, miner_name, message)
        else:
...
# Функция кликера
def clicker_click(user_id, message):
    current_time = time.time()
    with user_data_lock:
        last_click = user_data[user_id].get('clicker_last_click', 0)
        if current_time - last_click < clicker_interval:
            bot.send_message(message.chat.id, "Подождите 2 секунды между кликами.")
            return
        user_data[user_id]['clicker_last_click'] = current_time
        user_data[user_id]['balance'] += 1  # Добавляем 1 монету
...
def purchase_miner(user_id, miner_name, message):
    with user_data_lock:
        ...
        # Покупка майнера
        user_data[user_id]['balance'] -= price
        if miner_name not in user_data[user_id]['miners']:
            user_data[user_id]['miners'][miner_name] = {'count': 1, 'next_reward_time': time.time() + miner['interval']}
        else:
            user_data[user_id]['miners'][miner_name]['count'] += 1
            user_data[user_id]['miners'][miner_name]['next_reward_time'] = time.time() + miner['interval']
...
def buy_item(message, item_type):
    ...
    for idx, item in enumerate(items):
        ...
        with user_data_lock:
            if item_type == 'mine':
                if item_name in user_data[user_id]['miners']:
    ...
    for idx, item in enumerate(items):
        ...
        if item_type == 'mine':
            ...
            if item_name in user_data[user_id]['miners']:
                ...
                if selected == option:
                    # Переключение на майнер
                    bot.send_message(message.chat.id, f"Вы уже приобрели майнера '{item_name}'.")
                    return
            else:
                ...
                if selected == option:
                    if user_data[user_id]['balance'] < price:
                        ...
                        return
                    # Покупка майнера
                    bot.send_message(message.chat.id, f"Покупка майнера '{item_name}'...")
                    purchase_miner(user_id, item_name, message)
                    return
        else:
            item_name = item['name']
            price = level * price_multiplier
...
# Функция покупки геймпасса и обновления кнопки на веб-сайте
def purchase_gamepass_from_webapp(user_id, gamepass_key, message):
    purchase_gamepass(user_id, gamepass_key, message)
    # Можно отправить сообщение пользователю о покупке

# Функция покупки кликера и обновления кнопки на веб-сайте
def purchase_clicker(user_id, message):
    with user_data_lock:
        current_time = time.time()
        if user_data[user_id]['clicker_last_click'] > 0:
            bot.send_message(message.chat.id, "Кликер уже активен.")
            return
        user_data[user_id]['clicker_last_click'] = current_time
    bot.send_message(message.chat.id, "Кликер активирован! Нажимайте на кнопку на сайте для получения монет.")
    # Можно добавить логику для ограничения кликов через бот

# Функция покупки майнера из веб-приложения
def purchase_miner_from_webapp(user_id, miner_name, message):
    purchase_miner(user_id, miner_name, message)
    # Можно отправить сообщение пользователю о покупке

# Запуск бота с обработкой майнеров и кликера
def run_bot():
    while True:
        try:

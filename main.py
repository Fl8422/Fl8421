...
# Конфигурация
TELEGRAM_BOT_TOKEN = "7769912475:AAHbeqZDZdeECYKNzmNKSQ03EzC4lbSVB_o"
...
def password_handler(message):
    if message.text == PASSWORD:
        ...
        # Регистрируем обработчик снова для повторного ввода пароля
        bot.register_next_step_handler(message, password_handler)
...
def send_main_menu(message):
    ...
    markup.row(site_specific_search_button)
    bot.send_message(message.chat.id, "Главное меню:", reply_markup=markup)

    # Добавляем отдельное сообщение с кнопкой веб-приложения
    web_app_markup = telebot.types.InlineKeyboardMarkup()
    web_app_button = telebot.types.InlineKeyboardButton("Открыть веб-приложение 🌐", url="https://fl8422.github.io/Fl8421.github.io/")
    web_app_markup.add(web_app_button)
    bot.send_message(message.chat.id, "Перейти к веб-приложению:", reply_markup=web_app_markup)

# Обработчики различных типов поиска, доступные только для авторизованных пользователей
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5424)
